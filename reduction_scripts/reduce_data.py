...
def main():
    ...
    def run_extract_spectra(
        metadata_blue, metadata_red, prev_suffix, curr_suffix, **args
    ):
        # read the cubes
        sci_obs_list_blue = get_primary_sci_obs_list(metadata_blue)
        sci_obs_list_red = get_primary_sci_obs_list(metadata_red)
        for fn_blue, fn_red in zip(sci_obs_list_blue, sci_obs_list_red):
            in_blue = os.path.join(out_dir_blue, "%s.p%s.fits" % (fn_blue, prev_suffix))
            in_red = os.path.join(out_dir_red, "%s.p%s.fits" % (fn_red, prev_suffix))
            out_blue = os.path.join(out_dir_blue, "%s.p%s.fits" % (fn_blue, curr_suffix))
            out_red = os.path.join(out_dir_red, "%s.p%s.fits" % (fn_red, curr_suffix))
            
            # Define the pixel scale from the header
            f = pyfits.open(in_blue)
            binning2 = int(f[0].header["CCDSUM"][2])
            pixel_scale_x = 1  # arcsec/pix
            pixel_scale_y = 1 / 2 * binning2  # arcsec/pix

            # Extraction routine
            auto_extract(in_blue, in_red, out_blue, out_red, pixel_scale_x, pixel_scale_y, **args)
    ...
    # Load paths
    out_dir_blue = os.path.join(working_dir, "reduc_blue")
    out_dir_red = os.path.join(working_dir, "reduc_red")

    # Call here run_extract_spectra
    step_name = extract_params["step"]
    step_run = extract_params["run"]
    step_prev_suffix = extract_params["prev_suffix"]
    step_suffix = extract_params["suffix"]
    step_args = extract_params["args"]
    func_name = "run_" + step_name
    func = locals()[func_name]
    if step_run:
        func(
