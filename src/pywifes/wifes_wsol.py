...
def find_lines_and_guess_refs(
    ...
    # full xcorr version
        ...
        if plot:
            plot_detected_lines(
                chosen_slitlet, iter_x_array, iter_y_array, iter_ref_array, ncols
            )
...
def _xcorr_shift_all(packaged_args):
    ...
    for line, j in enumerate(this_ref_arc[:, 0]):
        ...
        if best_shift <= ncols:
            ...
            final_x_bes[best_shift - ncols :] = pseudo_x_bes[0 : -(best_shift - ncols)]
            final_lam_bes[best_shift - ncols :] = pseudo_lam_bes[
                0 : -(best_shift - ncols)
            ]
    ...
    while not finished:
        ...
        if automatic > 0:
            lines = om.excludeAuto(
                lines, grating, bin_x, bin_y, resid, sigma, doplot, verbose
            )
            alls, ally, allx, allarcs = om.extractArrays(lines, grating, bin_x, bin_y)
            err = numpy.ones_like(ally)
...
def derive_wifes_optical_wave_solution(
    ...
    allx, ally, alls, allarcs, params = _fit_optical_model(
        title,
        grating,
        bin_x,
        bin_y,
    ...
    if not (params is None):
        ...
        om.saveResamplingData(
            outfn, yrange, grating, bin_x, bin_y, params, halfframe=halfframe
        )
...
def derive_wifes_wave_solution(inimg, out_file, method="optical", **args):
    if method == "poly":
        derive_wifes_polynomial_wave_solution(inimg, out_file, **args)
    elif method == "optical":
