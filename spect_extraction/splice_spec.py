...
# In the reduce data we should add a validator for this based on the header parameter 'ARM' that can be 'Red' or 'Blue'
# The routine fails if the spectrum is not properly identified (i.e. having the red spectrum in blue_spec_path )

# We also nee validation for the aperture number

blue_spec_path = glob.glob('*.p12.fits')[0]
red_spec_path =glob.glob('*.p12.fits')[1]




blue_spect = SingleSpec(blue_spec_path) 
red_spect = SingleSpec(red_spec_path) 
import matplotlib.pyplot as plt
plt.ion(),plt.show()
plt.close('all')

plt.figure(1)
plt.plot(blue_spect.wl,blue_spect.flux)
plt.plot(red_spect.wl,red_spect.flux)





splice(blue_spec_path,red_spec_path)


# 2023rve
splice_path = glob.glob('*.p13.fits')[0]




spec = SingleSpec(splice_path) 
