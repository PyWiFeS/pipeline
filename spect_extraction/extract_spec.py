...
def auto_extract(blue_cube_path, red_cube_path, sky_sub=False, check_plot=False):
    ...
    border_width = 2  # Exclueded pixels atd the edge
    ...
    for index, source_ap in enumerate(source_aps):
        ...
        blue_flux, blue_var = spect_extract(
            blue_sci, b_var, source_aps[0], sky_ap=sky_aps[0]
        )
        red_flux, red_var = spect_extract(
            red_sci, r_var, source_aps[0], sky_ap=sky_aps[0]
        )
